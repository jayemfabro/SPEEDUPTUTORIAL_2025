...
const DetailsCalendarModal = ({
  ...
  const studentDropdownRef = React.useRef(null);
  
  // Update when selectedEvent changes
  React.useEffect(() => {
    if (selectedEvent) {
      // No need to handle array of student names anymore since each group class student
      // now has its own individual card
  ...
  const getCurrentStatusColor = (status) => {
    switch (status) {
      ...
      case 'Valid for Cancellation':
        return { bgColor: "bg-gray-200", borderColor: "border-gray-300" };
      case 'Free Class':
      ...
      case 'Absent Without Notice':
        return { bgColor: "bg-red-500", borderColor: "border-red-600" };
      default:
        return { bgColor: "bg-gray-400", borderColor: "border-gray-500" };
    }
  };

  // Helper function to get status information (matching legend)
    ...
    switch (status) {
      ...
      case 'Valid for Cancellation':
        return { 
          ...
          icon: <CalendarIcon className="h-4 w-4 text-gray-700" /> 
        };
      case 'Free Class':
      case 'FC not consumed':
        return { 
      ...
      default:
        return { 
          ...
          icon: <CalendarIcon className="h-4 w-4 text-slate-600" /> 
        };
    }
  };

    // Helper function to get current status - simplified as we don't have arrays of students anymore
    const getCurrentStudentStatus = () => {
        return selectedEvent.status;
    };

    const currentStatus = getCurrentStudentStatus();
    const statusInfo = getStatusInfo(currentStatus);  return (
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-1 mt-1">
                              <span className={`px-2 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${
                                (selectedEvent.classType || selectedEvent.class_type) === 'Premium' ? 'bg-orange-500 text-orange-800 border border-orange-200' :
                                (selectedEvent.classType || selectedEvent.class_type) === 'Group' ? 'bg-purple-100 text-purple-800 border border-purple-200' :
                                'bg-navy-100 text-navy-800 border border-navy-200'
                              }`}>
                                {selectedEvent.classType || selectedEvent.class_type || 'Unknown'}
                              </span>
                            </div>
                          </div>
                        </div>
                            ...
                            <div className="relative z-10" ref={dropdownRef}>
                              ...
                              >
                                <div className="flex items-center min-w-0 flex-1">
                                  <span className={`w-3 h-3 ${getCurrentStatusColor(currentStatus).bgColor} ${getCurrentStatusColor(currentStatus).borderColor} border flex-shrink-0 rounded-sm mr-2`}></span>
                                  <span className="font-medium text-xs truncate">{currentStatus || 'Select Status'}</span>
                                </div>
                                      ...
                                      {statusOptions.map((status) => (
                                        <button
                                          ...
                                          onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            console.log('Status clicked:', status.value);
                                            if (onStatusChange) {
                                              onStatusChange(selectedEvent.id, status.value);
                                            }
                                            setStatusDropdownOpen(false);
                                          }}
                                        >
